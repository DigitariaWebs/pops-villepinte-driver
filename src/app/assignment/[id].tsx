...
export default function AssignmentScreen(): React.ReactElement {
  const router = useRouter();
  const { id } = useLocalSearchParams<{ id: string }>();

  const [assignment, setAssignment] = useState<RemoteAssignment | null>(null);
  ...
  const respond = useCallback(
    async (status: "accepted" | "refused") => {
      ...
      try {
        ...
        const updated = await respondToAssignment(id, status, note);
    ...
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 24, gap: 16 }}>
        ...
        <Card>
          ...
          {order?.notes ? (
            <>
              <Label style={{ marginTop: 10 }}>Note client</Label>
              <Value muted>{order.notes}</Value>
            </>
          ) : null}
        </Card>
...
function StatusPill({
  ...
  const labels: Record<RemoteAssignment["status"], string> = {
    pending: "EN ATTENTE DE TA RÉPONSE",
    accepted: "ACCEPTÉE",
    refused: "REFUSÉE",
    cancelled: "ANNULÉE PAR LE RESTAURANT",
  };
  const bg: Record<RemoteAssignment["status"], string> = {
    pending: colors.primary,
    accepted: colors.success,
    refused: colors.danger,
    cancelled: colors.border,
  };
  const fg: Record<RemoteAssignment["status"], string> = {
    pending: colors.ink,
